...
const Auth: NextPage = () => {
  ...
  //LoginボタンまたはRegisterボタンをクリックした時に実行させる関数を用意する。
  ...
  return (
    <Layout title="Auth">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <input //password入力用のpasswordフォームを用意。
            type="password"
            required
            className="my-2 rounded border border-gray-300 px-3 py-2 text-sm  placeholder-gray-500 focus:border-indigo-500 focus:outline-none"
        ...
        <div className="my-6 flex items-center justify-center text-sm">
          <span
            onClick={() => setIsLogin(!isLogin)} //loginモードとregisterモードを切り替えるためのspanタグを用意。
            className="cursor-pointer font-medium hover:text-indigo-500"
          >
            change mode ?
          </span>
        </div>
        <button //submitボタンを用意する。
          type="submit"
